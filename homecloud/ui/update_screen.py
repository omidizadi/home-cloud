...
class UpdateScreen(Screen):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        bid = event.button.id
        # thread=True keeps subprocess.run off the event loop so the log repaints live.
        if bid == "btn-self":
            self.run_worker(self._update_self, thread=True)
        elif bid == "btn-nc":
            self.run_worker(self._update_nextcloud, thread=True)
        elif bid == "btn-bot":
            self.run_worker(self._update_bot, thread=True)
        elif bid == "btn-restic":
            self.run_worker(self._update_restic, thread=True)
        elif bid == "btn-samba":
            self.run_worker(self._update_samba, thread=True)
        elif bid == "btn-all":
            self.run_worker(self._update_all, thread=True)
        elif bid == "btn-back":
            self.app.pop_screen()
    ...
    async def _confirm(self, prompt: str) -> bool:
        ...
        return await confirm(self.app, prompt)
    ...
    async def _update_self(self) -> None:
        try:
            ...
            self._log(f"[red bold]💥 Update failed: {e}[/]")
            log.exception("_update_self failed")
    ...
    async def _update_nextcloud(self) -> None:
        try:
            ...
            self._log(f"[red bold]💥 Update failed: {e}[/]")
            log.exception("_update_nextcloud failed")
    ...
    async def _update_bot(self) -> None:
        try:
            ...
            self._log(f"[red bold]💥 Update failed: {e}[/]")
            log.exception("_update_bot failed")
    ...
    async def _update_restic(self) -> None:
        try:
            ...
            self._log(f"[red bold]💥 Update failed: {e}[/]")
            log.exception("_update_restic failed")
    ...
    async def _update_samba(self) -> None:
        try:
            ...
            self._log(f"[red bold]💥 Update failed: {e}[/]")
            log.exception("_update_samba failed")
